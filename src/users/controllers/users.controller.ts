...
export class UsersController {
  ...
  async Login(@Body() body: LoginDto) {
    const user = await this.usersService.FindOneEmail(body.email);
  ...
  async Delete(@Param("id") id: string, @Req() req): Promise<{}> {
    ...
    if (Role.Admin != me.role)
      throw new HttpException("Vous n'êtes pas administrateur", 403);
  ...
  async Update(
    @Param("id") id: string,
    @Req() req,
    @Body() body: UpdatedUsersDto,
  ): Promise<{}> {
    const me = await this.usersService.FindOneId(req.user.id);
    if (!me) throw new UnauthorizedException();
    if (Role.Admin != me.role && me.id != id)
      throw new HttpException("Tu n'es pas administrateur", 403);
    if (body.role === "Admin" && me.role !== Role.Admin)
      throw new HttpException(
        "Tu n'es pas administrateur pour réaliser cette action",
        403,
      );
